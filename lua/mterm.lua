...
local update_title = function()
  if M.size() == 0 or not M.win:is_open() then return end -- last buf bdeleted
  local win = assert(M.win:get_win())
  if M.win.opts.layout == 'bot' then -- setlocal stl require laststatus~=3
    vim.wo[win].winbar = build_winbar()
  else
    vim.wo[win].winbar = ''
    api.nvim_win_set_config(win, { title = build_title() })
  end
end
...
M.switch = function(term)
  if curr == term then return end
  curr = term
  if not M.win:is_open() then return end -- when `:quit!`, win is not valid
